...
def b_fitting(pos, neg, zero, d, plot = True):
    ...
    if plot:
        ...
        b_pos = np.exp((pos_closest_t * R_pos_fit[0] + R_pos_fit[1])/((pos_closest_t * R_pos_fit[0] + R_pos_fit[1]) - 
                                                                      R_pos[pos_closest_t_ind]))
        b_neg = np.exp((neg_closest_t * R_neg_fit[0] + R_neg_fit[1])/((neg_closest_t * R_neg_fit[0] + R_neg_fit[1]) - R_neg[neg_closest_t_ind]))
